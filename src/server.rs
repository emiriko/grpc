        ...
        tokio::spawn(async move {
            for i in 0..30 {
                if tx.send(Ok(TransactionResponse {
                    transaction_id: format!("transaction_{}", i),
        ...
        tokio::spawn(async move {
            while let Some(message) = stream.message().await.unwrap_or_else(|_| None) {
                ...
                let reply = ChatMessage {
                    ...
                    message: format!("Terima kasih telah melakukan chat kepada CS virtual. Pesan anda akan dibalas pada jam kerja. Pesan yang diterima: {}", message.message)
                };
